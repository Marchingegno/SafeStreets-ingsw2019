...
one sig ON_DEVICE extends ViolationReportState {}
one sig ON_NETWORK extends ViolationReportState {
	encryptedConnection: one Bool
}
one sig ON_SERVER extends ViolationReportState {}
...
fact requirement4_5_6_7_8 {
	all v : ViolationReport | v.state = ON_DEVICE implies 
(#v.pictures >= 1 and 
 #v.location = 1 and 
 #v.timestamp = 1 and 
...
(#v.pictures >= 1 and 
 #v.location = 1 and 
 #v.timestamp = 1 
 and #v.typeOfViolation = 1 
 and #v.licensePlate = 1)
}
check goal2 for 5
